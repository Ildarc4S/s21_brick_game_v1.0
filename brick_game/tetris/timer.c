...
void _updateLastTime(Timer_t *this) { this->last_time = this->current_time; }
...
void _setTimerTick(Timer_t *this, long tick) { this->tick = tick; }

Timer_t constructorTimer() {
  struct timeval time = {0};
  gettimeofday(&time, NULL);

  return (Timer_t){.current_time = time,
                   .last_time = time,
